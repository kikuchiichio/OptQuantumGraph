...
//  printf( " det*det=%lf ",det);
//  cout<<U[B]<<" "<<U[0]<<endl;
// The square norms on each edge;
  double SQNORM[B+1],SQNORMFULL;
  SQNORMFULL=0;
  for(j=1;j<=B;j++)
  {
    double kk=XIN[0];
    complex<double> ca,cb;
    ca=U[0]-U[j]*complex<double>(cos(kk*L[j]),sin(kk*L[j]));
    ca/=1.-complex<double>(cos(2*kk*L[j]),2*sin(kk*L[j]));
    cb=U[0]-U[j]*complex<double>(cos(kk*L[j]),-sin(kk*L[j]));
    cb/=1.-complex<double>(cos(2*kk*L[j]),-2*sin(kk*L[j]));

    complex<double> tb;
   tb=ca*conj(cb)*(complex<double>(cos(kk*L[j]),sin(kk*L[j]))-1.);
    tb /= (complex<double>(0,1));
    tb /= kk;
    SQNORM[j]=(abs(ca)*abs(ca)+abs(cb)*abs(cb))*L[j]+2*real(tb);
    SQNORMFULL+=SQNORM[j];
    //printf(":%d %lf ",j,SQNORM[j]);
  }
  //printf("\n");

  return  SQNORM[1]/SQNORMFULL;
}
